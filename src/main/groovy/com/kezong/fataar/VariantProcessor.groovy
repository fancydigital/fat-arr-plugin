...
class VariantProcessor {
    ...
    private Collection<Task> mMergeTasks = new ArrayList<>();
    ...
    private TaskProvider handleRefactorClasses(Map<String, String> refractoryMap,
                                                Map<String, String> replaceClassPath,
                                               List<String> deleteClasses) {
        println "task ==> handleRefactorClasses"

        Map<String, String> refactorPaths = new HashMap<>()

        refractoryMap.forEach {oldPkgName, newPkgName ->
            refactorPaths.put(
                    oldPkgName.replace('/','.'),
                    newPkgName.replace('/', '.')
            )
        ...
        deleteClasses.forEach { deletePath ->
            deletePaths.add(deletePath.replace('/', '.'))
        }

        final TaskProvider task = mProject.tasks.register("refractoryClasses" + mVariant.name.capitalize()) {
            dependsOn(mExplodeTasks)
            dependsOn(mMergeTasks)
            dependsOn(mVersionAdapter.getJavaCompileTask())



            doLast {
                List<File> javacDirs = mVersionAdapter.getClassPathDirFiles().asList()
                if(javacDirs == null || javacDirs.isEmpty()) {
                    println "[PACKAGE_NAME_REFACTOR] not need refactor"
                } else {
                    ExplodedHelper.processRefractoryClasses(
                            mProject,
                            refactorPaths,
                            replaceClassPath,
                            deletePaths,
    ...
    private void processClassesAndJars(TaskProvider<Task> bundleTask) {
        ...
        FatAarExtension extension = mProject.extensions.findByType(FatAarExtension.class)
        // 处理配置 重构流程的代码
        TaskProvider refactorClasses = null
        println("apply fat library , extension ${extension.replacePackagePath}")

        if(extension != null) {
            if(extension.replacePackagePath != null && !extension.replacePackagePath.isEmpty()) {
                refactorClasses = handleRefactorClasses(
                        extension.replacePackagePath,
                        extension.replaceClassPath,
                        extension.deleteClasses)
            }
        }

        println("[OUTTASK] merge task ${mergeClasses.name}")
        syncLibTask.configure {
            dependsOn(mergeClasses)
            if(refactorClasses != null) {
                dependsOn(refactorClasses)
            }
            inputs.files(mAndroidArchiveLibraries.stream().map { it.libsFolder }.collect())
                    .withPathSensitivity(PathSensitivity.RELATIVE)
            inputs.files(mJarFiles).withPathSensitivity(PathSensitivity.RELATIVE)
